# SlopProof documentation

The root README explains the product and local demo. The documents here cover
the public contracts needed to evaluate, operate, or contribute to SlopProof.

## Product status

- [Project status](project-status.md)
- [Self-hosting](operations/self-hosting.md)
- [Dogfooding the required check](maintainers/dogfooding.md)

## Security and privacy

- [Threat model](security/threat-model.md)
- [Recording cryptography](recording-crypto-v1.md)
- [Provider data flow](privacy/provider-data-flow.md)
- [Incident response](security/incident-response.md)
- [Key rotation](operations/key-rotation.md)
- [R2 CORS and lifecycle](operations/r2-cors-lifecycle.md)

## Operations

- [Production configuration](operations/production-configuration.md)
- [Deployment and rollback](operations/production-deployment.md)
- [Database backup and restore](operations/database-backup-restore.md)
- [Evidence-free observability](operations/observability.md)
- [Public release checklist](maintainers/public-release-checklist.md)

Contributor expectations live in [CONTRIBUTING.md](../CONTRIBUTING.md).
Vulnerabilities belong in GitHub private vulnerability reporting as described
in [SECURITY.md](../SECURITY.md).
