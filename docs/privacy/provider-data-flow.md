...
| Recipient             | Sent                                                                                                                                                 | Explicitly not sent                                                                                                                                  | Persistence in UnderstandProof                                                                      |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
...
| OpenRouter STT        | One bounded per-question mono 16-kHz WAV after Worker-only decryption                                                                                | Full video, other answers, patch, repository, author identity, public object URL                                                                     | Only encrypted question-bound transcript is stored by UnderstandProof                               |
| OpenRouter generation | Bounded untrusted patch context for Learning and Proof questions; one private Practice answer for feedback                                           | Full video, transcript, frames, GitHub/OAuth identity, public object URL, Proof answers in Practice                                                  | Learning and Practice artifacts are encrypted worker-only; Proof questions are frozen per attempt   |
...
| Hetzner inference     | The same minimized generation or judge material only after an eligible OpenRouter transport failure                                                  | Full video, public Evidence URL, GitHub/OAuth identity, face/person metadata, room/accent/disability/tool-analysis fields, Practice answers in Proof | Learning/Practice/Judge artifacts are encrypted worker-only; the public Check has none              |

Provider calls disable tools and request `store=false` where the endpoint
supports it. OpenRouter generation and judge routing additionally require
supported request parameters, denied data collection and a ZDR-capable endpoint.
Requests and responses have absolute deadlines and byte caps; transport retry is
